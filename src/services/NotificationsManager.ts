...
export default class NotificationsManager {
    ...
    constructor(store: Store) {
        ...
        const magicBellApiUrl = config.get("magicBellApiUrl") || "https://api.magicbell.com";
        if (!githubUsername || !magicBellApiKey) {
            throw new Error("Missing configuration for CodeInbox extension. Please set your GitHub username and MagicBell API key in the settings.")
        }
        this.store = store;
        this.magicBell = new MagicBell({
            apiKey: magicBellApiKey,
            userExternalId: "moxley01",
    ...
    public async init() {
        ...
        this.magicBell.listen().forEach(async event => {
            const newNotification = await this.magicBell.notifications.get(event.data.id)
